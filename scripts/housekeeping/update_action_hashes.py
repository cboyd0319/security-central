...
def get_commit_sha(repo: str, ref: str) -> tuple:
    ...
    result = subprocess.run(
        ["gh", "api", f"repos/{repo}/commits/{ref}", "--jq", ".sha"], capture_output=True, text=True
    )
    ...
    if result.returncode == 0:
        ...
        tag_result = subprocess.run(
            ["gh", "api", f"repos/{repo}/git/ref/tags/{ref}", "--jq", ".object.sha"],
            capture_output=True,
            text=True,
        )
...
def update_sync_script():
    ...
    # Read current script
    with safe_open(script_path, allowed_base=False) as f:
        content = f.read()
    ...
    new_dict += "}"  # PERFORMANCE: Use list and join()
    ...
    # Write back
    with safe_open(script_path, "w", allowed_base=False) as f:
        f.write(new_content)
