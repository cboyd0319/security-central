...
from generate_repo_matrix import generate_matrix, generate_tech_matrix, main, print_matrix_info
...
class TestPrintMatrixInfo:
    ...
    def test_print_matrix_info_shows_technologies(self, sample_repos_file, capsys):
        ...
        assert (
            "python" in output.lower() or "javascript" in output.lower() or "tech" in output.lower()
        )
...
class TestMain:
    ...
    def test_main_default_behavior(self, sample_repos_file, capsys):
        ...
        with patch("generate_repo_matrix.generate_matrix") as mock_generate:
            ...
            # Main returns normally when successful (doesn't call sys.exit)
            try:
                main()
            except SystemExit:
