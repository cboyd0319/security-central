...
from dependency_analyzer import DependencyRisk, SupplyChainAnalyzer
...
class TestDependencyRisk:
    ...
    def test_to_dict(self):
        ...
        risk = DependencyRisk(
            name="django", version="4.0.0", risk_score=15.0, issues=["missing_security_policy"]
        )
    ...
    def test_empty_issues(self):
        ...
        risk = DependencyRisk(name="safe-package", version="1.0.0", risk_score=0.0, issues=[])
...
class TestSupplyChainAnalyzer:
    ...
    def test_analyze_python_deps_no_risks(
        self, mock_typosquat, mock_metadata, mock_subprocess, analyzer
    ):
        """Test analyzing dependencies with no risks."""
        # Mock pip list output
        mock_result = Mock()
        mock_result.stdout = json.dumps(
            [{"name": "requests", "version": "2.28.0"}, {"name": "django", "version": "4.0.0"}]
        )
    ...
    @patch.object(SupplyChainAnalyzer, "_get_pypi_metadata")
    @patch.object(SupplyChainAnalyzer, "_is_typosquat")
    def test_analyze_python_deps_with_typosquat(
        self, mock_typosquat, mock_metadata, mock_subprocess, analyzer
    ):
        ...
        mock_metadata.return_value = {"info": {"maintainers": ["user1"], "project_urls": {}}}
    ...
    def test_get_pypi_metadata_success(self, analyzer):
        ...
        with patch.object(analyzer.session, "get") as mock_get:
            ...
            mock_response.json.return_value = {"info": {"name": "requests", "version": "2.28.0"}}
    ...
    def test_is_typosquat_exact_match(self, analyzer):
        ...
        assert analyzer._is_typosquat("requests")  # Use if not var: instead
        assert analyzer._is_typosquat("django")  # Use if not var: instead
        assert analyzer._is_typosquat("flask")  # Use if not var: instead
...
class TestSupplyChainAnalyzerIntegration:
    ...
    def test_filtering_high_risk_only(self, mock_metadata, mock_subprocess, temp_repo):
        ...
        mock_result = Mock()
        mock_result.stdout = json.dumps(
            [{"name": "low-risk", "version": "1.0.0"}, {"name": "high-risk", "version": "0.1.0"}]
        )
        mock_subprocess.return_value = mock_result
        ...
        def metadata_side_effect(package_name):
            ...
            if package_name == "low-risk":
                ...
                return {"info": {"maintainers": ["solo"], "project_urls": {}}}
...
class TestDependencyRiskDataclass:
    ...
    def test_dataclass_fields(self):
        ...
        risk = DependencyRisk(name="test", version="1.0", risk_score=50.0, issues=[])
    ...
    def test_to_dict_structure(self):
        ...
        risk = DependencyRisk(
            name="package", version="2.0.0", risk_score=75.5, issues=["issue1", "issue2"]
        )
    ...
    def test_multiple_issues(self):
        ...
        issues = ["typosquatting", "no_github_repo", "single_maintainer", "uses_exec_eval"]
        risk = DependencyRisk(
            name="suspicious-pkg", version="0.0.1", risk_score=85.0, issues=issues
        )
    ...
    def test_risk_score_float_precision(self):
        ...
        risk = DependencyRisk(name="test", version="1.0", risk_score=33.333333, issues=[])
